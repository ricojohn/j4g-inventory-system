...
@section('content')
    ...
    @php
        $user = auth()->user();
        $categoriesHref = $user?->can('view categories') ? route('categories.index') : null;
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <x-ui.stat-card label="Total Categories" :value="$totalCategories" :href="$categoriesHref" data-dashboard-stat="total-categories" />
        ...
        <x-ui.stat-card label="Low Stock Items" :value="$lowStockCount" accent="warning" :href="$lowStockHref" data-dashboard-stat="low-stock-count" />
        <x-ui.stat-card label="Out of Stock Items" :value="$outOfStockCount" accent="danger" :href="$outOfStockHref" data-dashboard-stat="out-of-stock-count" />
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    const table = initAsyncTable({
        ...
        renderRows: (rows) => rows.map((movement) => `
            <tr>
                ...
                <td>${escapeHtml(movement.product_name)} (${escapeHtml(movement.color)})</td>
    ...
    window.addEventListener('inventory:updated', async () => {
        try {
            ...
            // Ignore refresh errors; Pusher event still updated other UI.
        }
    });
});
</script>
